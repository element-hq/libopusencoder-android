...
extern "C"
JNIEXPORT jint JNICALL Java_io_element_android_opusencoder_OggOpusEncoderImpl_init(JNIEnv *env, jobject thiz, jstring file_path, jint sample_rate) {
    char *path = (char*) env->GetStringUTFChars(file_path, 0);
    return oggCodec.encoderInit(path, sample_rate);
}

extern "C"
JNIEXPORT jint JNICALL Java_io_element_android_opusencoder_OggOpusEncoderImpl_writeFrame(JNIEnv *env, jobject thiz, jshortArray shorts, jint samples_per_channel) {
    jshort *nativeShorts = env->GetShortArrayElements(shorts, 0);
    return oggCodec.writeFrame((short *) nativeShorts, samples_per_channel);
}

extern "C"
JNIEXPORT jint JNICALL Java_io_element_android_opusencoder_OggOpusEncoderImpl_setEncoderBitrate(JNIEnv *env, jobject thiz, jint bitrate) {
